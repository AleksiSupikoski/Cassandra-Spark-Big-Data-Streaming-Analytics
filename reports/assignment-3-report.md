...
The key logic of data processing in code above is that it tests for occurrence of null's in case of wrong data being sent, for another dataframe i test if environmental variables are suitable for the tortoise, in the main df, i declare a 1-minute sliding window with watermark and calculate average values for temperature, humidity, acceleration etc. and also metrics are calculated for every window, for latency i use the last timestamp and current time, and for calculating throughput i use window lenght and number of rows processed, i then aggregate them by dev-id. Then the main dataframes data is every minute sent back to tenant's topic consumed by daas, i decided not to implement coredms ingestion, but it is as simple as writing to kafka with pyspark's functionality. When environmental metrics don't comply with supposed healthly environment for tortoise, i send them to another kafka topic which warns the tenant when he consumes it from daas with their application. When data is wrong an empty json is sent there.
...
In order to achieve end-to-end exactly once delivery, we would need to set the kafka broker to send data exactly once. Additionally in my case i have noticed that some events were dropped from the dataframe, due to performance issues, resulting in slow processing time: the processing time for each batch is too slow, it had lead to a backlog of data, which resulted in missed or dropped events (rows). Allocating more workers and running on powerful enough machine as well as setting kafka topic to exactly once and ensuring reliable network at every stage and a reliable IoT data producing device wouldvresult in absolute exactly once delivery stream, however interruptions are always possible no matter how we configure our system due to unexpected reasons, therefore exactly once delivery is impossible in real life.
