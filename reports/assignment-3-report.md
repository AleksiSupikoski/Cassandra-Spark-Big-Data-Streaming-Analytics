...
We would need to set the kafka broker to send data exactly once. Additionally in my case i have noticed that some events were dropped from the dataframe, due to performance issues, resulting in slow processing time: the processing time for each batch is too slow, it had lead to a backlog of data, which resulted in missed or dropped events (rows). Allocating more workers and running on powerful enough machine as well as setting kafka topic to exactly once and ensuring reliable network at every stage and a reliable IoT data producing device wouldvresult in absolute exactly once delivery stream.
